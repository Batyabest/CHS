...
class officeModExtraController extends officeDefaultController
{
    ...
    {
        if (defined('MODX_ACTION_MODE') && MODX_ACTION_MODE && !empty($_SESSION['Office']['CHS'])) {
            $this->config = $_SESSION['Office']['CHS'];
            $this->config['json_response'] = true;
        } else {
            $this->config = array_merge(array(
                'tplOuter' => 'tpl.CHS.office',
            ), $config);

            $_SESSION['Office']['CHS'] = $this->config;
        }

        $this->office->config['processorsPath'] = MODX_CORE_PATH . 'components/chs/processors/office/';
    }
    ...
    public function defaultAction()
    {
        ...
        */
        ...
        $css = trim($this->modx->getOption('office_chs_frontend_css', null,
            MODX_ASSETS_URL . 'components/office/css/main/default.css', true));
        if (!empty($css)) {
            $this->modx->regClientCSS(str_replace($config['pl'], $config['vl'], $css));
        }

        $js = trim($this->modx->getOption('office_chs_frontend_js', null,
            MODX_ASSETS_URL . 'components/chs/js/office/default.js'));
        if (!empty($js)) {
            ...
            $this->office->addClientJs(array(
                MODX_ASSETS_URL . 'components/chs/js/mgr/chs.js',
                MODX_ASSETS_URL . 'components/chs/js/mgr/misc/utils.js',
                MODX_ASSETS_URL . 'components/chs/js/office/home.panel.js',
                MODX_ASSETS_URL . 'components/chs/js/office/items.grid.js',
                MODX_ASSETS_URL . 'components/chs/js/office/items.windows.js',
    ...
    {
        $corePath = $this->modx->getOption('chs.core_path', null,
            $this->modx->getOption('core_path') . 'components/chs/');
        $modelPath = $corePath . 'model/';

        return $this->modx->addPackage('chs', $modelPath);
    }
